package com.kurtspencer.math.noise;
/*
...
public class OpenSimplexNoiseTileable3D {
	...
	public OpenSimplexNoiseTileable3D(int w6, int h6, int d6) {
		this(DEFAULT_SEED, w6, h6, d6);
	}
	
	public OpenSimplexNoiseTileable3D(short[] perm, int w6, int h6, int d6) {
		this.perm = perm;
		permGradIndex3D = new short[256];
		this.w6 = w6;
		this.h6 = h6;
		this.d6 = d6;
		sOffset = Math.max(w6, Math.max(h6, d6)) * 6;
	...
	public OpenSimplexNoiseTileable3D(long seed, int w6, int h6, int d6) {
		perm = new short[256];
		permGradIndex3D = new short[256];
	...
	{
		int bSum = xsb + ysb + zsb;
		int xc = (3 * xsb + bSum) / 18 / w6;
		int yc = (3 * ysb + bSum) / 18 / h6;
		int zc = (3 * zsb + bSum) / 18 / d6;
		
		int xsbm = (-5 * w6 * xc) + (h6 * yc) + (d6 * zc) + xsb;
		int ysbm = (w6 * xc) + (-5 * h6 * yc) + (d6 * zc) + ysb;
		int zsbm = (w6 * xc) + (h6 * yc) + (-5 * d6 * zc) + zsb;
		
		int index = permGradIndex3D[(perm[(perm[xsbm & 0xFF] + ysbm) & 0xFF] + zsbm) & 0xFF];
